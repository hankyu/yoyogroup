...
$myOrderID = count($sign_id);
	...
	$ss=chk_status($act);
	if(trim($sign_id[$i])!=""){
	$result_log = "INSERT INTO xfun_log (log_name,log_user,cdate)VALUES('[".$sign_id[$i]."-".view_kind($sign_id[$i],"xfun_order_list","order_id","order_name")."]歷史訂單資料".$ss."','".$sec_id."','".date("Y-m-d H:i:s")."')";
...
function chk_status($id)
{
	...
	case "A4":
		...
		break;
	case "A5":
		return "<font color=green>訂單成立</font>";
		break;
	case "A6":
		return "<font color=red>失效訂單</font>";
		break;
	case "A8":
		return "刪除訂單";
		break;
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <td height="22" align="left" valign="top">
          ...
          <select name="act" class="inputkeyword01" id="act" >
            ...
            <option value="A0" >重新處理</option>
            <option value="A8" >刪除訂單</option>
           </select>
