...
function chk_status($id)
{
	...
	case "A5":
		...
		break;
	case "A6":
		return "<font color=red>訂單失效 (未匯款)</font>";
		break;
	case "A8":
		return "刪除訂單";
		break;
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <td height="22" align="left" valign="top">
          ...
          <select name="act" class="inputkeyword01" id="act" >
            ...
            <option value="A6" >訂單失效 (未匯款)</option>
...
<?PHP
  $result_num = "SELECT * FROM $XFUN_TBL[TABLE_XFUN_xfun_order_list] ".$query_where." ORDER BY order_date ASC LIMIT $show,$limit";
  //echo $result_num;
