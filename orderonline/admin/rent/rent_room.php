...
$(function(){
	$('.checkDate').click(function(e){
		if($(this).attr("checked")){
			...
			//if($('#rentDateSpan').html()!=""){	datetime=", "+datetime;	}
			//var html = "<span class='displayDateTime' id='display"+datetime+"'>"+datetime+"</span>";
			var html = "<input type='text' id='display"+datetime+"' value='"+datetime+"' size='12' readonly >";
			$("#rentDateSpan").append(html);
		}
		else{
			var datetime = $(this).val();
			$('#display'+datetime).remove();
		}
	});
...
function check(theForm){
    ...
    if(document.theForm.rentDatePrice.value != ""){
		...
		if($('#rentDatePrice').val() < '<?php echo $XFUN_CONFIG['CONFIG_MIN_ROOM_PRICE'];?>'){
			alert("房價請勿輸入低於<?php echo $XFUN_CONFIG['CONFIG_MIN_ROOM_PRICE'];?>元");
	  		document.theForm.rentDatePrice.focus();
	  		return (false);
		}
  	}
  	...
  	if($('#rentDateAmount').val()==""||$('#rentDateAmount').val()==0){
		alert("開放數量不可為 0 或空值");
		document.theForm.rentDateAmount.focus();
		return (false);
  	}
...
<table width="100%"border="1" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" bordercolordark="#ffffff" bordercolorlight="#cccccc">
      ...
      <tr>
        <td width="603">設定日期：<!-- span id="rentDateSpan"></span -->
        	<span id="rentDateSpan"><input type="text" id="display<?=$rentDate?>" value="<?=$rentDate?>" size="12" readonly /></span>
          <!--input name="rentDate" type="text" id="rentDate" value="<?=$rentDate?>" readonly/>--></td>
      </tr>
      <tr>
        <td><span class="style5">房型定價</span>：
          <input name="rentDatePrice" type="text" id="rentDatePrice" value="<?=$rentPrice?>" size="12" /></td>
      </tr>
      <tr>
        <td>開放數量：
          <input name="rentDateAmount" type="text" id="rentDateAmount" value="0" size="12" /></td>
      </tr>
      <tr>
        <td align="left">
		<div style="width:15px; height:15px; padding-bottom:3px; background-color:#00CCFF; float:left; border:solid #333333 1px;"></div>
		<div style="height:20px; padding-bottom:3px; padding-left:10px; line-height:20px;">藍色表已設定資料。</div>
		<div style="width:15px; height:15px; background-color:#FFCC00; float:left; border:solid #333333 1px;"></div>
		<div style="height:20px; padding-left:10px; line-height:20px;"> 橙色表已有房間出租。</div></td>
      </tr>
      <tr>
...
?>
<?php if(!isset($rdn)): ?>
<form id="form1" name="form1" enctype="multipart/form-data" method="post" action="<?=$SubmitURL?>" >	
...
<table width="100%"border="1" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF" bordercolordark="#ffffff" bordercolorlight="#cccccc">
  <tr align="center" bgcolor="#065F93" class="style1">
    ...
    <td width="10%" bgcolor="#CCCCCC">最後編輯</td>
    <td width="10%" bgcolor="#CCCCCC">編輯</td>
    <td bgcolor="#CCCCCC"><input type="button" id="deleteSome" value="批次刪除" <?php echo $deleteSomeDisableStr;?>></td>
  </tr>
<?PHP
...
<img src="../images/edit.gif" alt="修改資料" width="16" height="16" border="0" align="middle"></a> <a href="JavaScript:chkdel('<?=$SubmitURL?>?act=delete&rentNum=<?=$id?>&rentDateNum=<?=$rentDateNum; ?>&PB_page=<?=$PB_page?>&main_page=<?=$main_page?>')"><img src="../images/del2.gif" alt="刪除資料" width="16" height="16" border="0" align="middle" style="margin-left:10px"></a>
<? endif;?>	  </td>
	<td><input type="checkbox" name="deleteItem[]" value="<?php echo $rentDateNum;?>" <?php echo $deleteSomeDisableStr;?>></td>
  </tr>  
</form>
<?
  }  //while_end
