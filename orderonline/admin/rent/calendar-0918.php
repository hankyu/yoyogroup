...
function showMonth ($calmonth,$calyear) {
	...
	echo "<font color='#ffffff'>".date("Y-m-d h:i:s")."</font>";
...
//echo "($d)".$days1."-".$days2;
			if($days1 >= $days2) {
				...
				/****該月份其他日期****/
				echo '<td class="day" id="'.GetRentDate($chkDate).'" '.$onmouseOver.$onmouseOut.'><div class="day_of_month"><a href="'.$day_link.'?id='.$id.'&PB_page='.$PB_page.'&main_page='.$main_page.'&c='.$c.'&m='.$calmonth.'&a='.$d.'&y='.$calyear.'">'. $d. '</a></div>';
